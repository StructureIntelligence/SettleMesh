...
The server exposes a capability-invoke tool over the same search→show→quote→call loop below: search for a tool, inspect it, quote the exact paid call, then invoke any catalog capability by id, with a confirm step before paid calls. The same Aev billing, quotes, and error contract apply. Run `settlemesh login` first to omit the key.
...
Use `--wait` for async jobs and `--confirm` for costly or side-effecting calls. `settlemesh tool call` remains a compatible alias, but new agents should teach and use `settlemesh call <entrypoint-id>`. Always parse JSON defensively. Result URLs or payloads may appear in `data.result`, `data.results`, `data.output`, `output`, `url`, `urls`, or nested arrays/objects.
...
**Quote before charging (recommended for every paid call):** CLI: `settlemesh quote <entrypoint-id> --input '{...}' --json`. HTTP: `POST /v1/billing/quote` with `{"capability_id":"..."}` or `{"agent_id":"..."}` or `{"app_id":"...","endpoint_id":"..."}` → `{base_cost_credits, markup_bps, multiplier, total_credits, markup_deduped, chain:{depth,max_depth}, payer:{delegated, allowance?}}`. Read-only (no hold). For routed or metered entries, `total_credits` is the hold ceiling; `hold_ceiling_credits` names that pre-authorization explicitly, and `actual_capture_may_be_lower` means the final ledger capture may be lower after the winning provider or actual usage is known. Show the pre-pay ceiling to the user before a costly action, then show the exact ledger/header charge after completion.
...
Note on visibility: `--visibility public` is **auto-approved and searchable immediately** once the mechanical gates all pass — every public operation needs at least one runnable `examples[].input`, copy must not name upstream vendors, flat pricing must clear the platform-cost floor, and the safety gate (allowed_hosts / positive pricing / abuse protection) must hold; any failure is a 422 that tells you exactly what to fix and resend. (Operators can flip publishing back to a manual approval queue via `SETTLEMESH_AUTO_APPROVE_PUBLIC_SERVICES=false`; check state with `settlemesh services list`.) Your own dynamic service is callable by you **right after `upload`, before it goes public**, but only via the raw HTTP invoke endpoint — the CLI `tool call` / `services show <dsvc_id>` do NOT resolve an unapproved dynamic service. Use the `dsvc_...` id from `upload`:
```
